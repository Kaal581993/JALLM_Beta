package com.jalllm.geforce.autograd.ops;

import com.jalllm.geforce.activation.GELU;
import com.jalllm.geforce.autograd.Operation;
import com.jalllm.geforce.autograd.Variable;
import com.jalllm.geforce.tensor.Tensor;

import java.util.List;

/** GELU activation: z = GELU(x) */
public final class GeluOp implements Operation {

    private final Variable x;
    private final Variable z;

    public GeluOp(Variable x) {
        this.x = x;
        Tensor result = GELU.forward(x.value());
        this.z = Variable.fromOp(result, this, "gelu");
    }

    @Override
    public List<Variable> inputs() {
        return List.of(x);
    }

    @Override
    public Variable output() {
        return z;
    }

    @Override
    public void backward(Tensor gradOutput) {
        // Use the existing GELU backward implementation
        if (x.requiresGrad()) {
            Tensor gradX = GELU.backward(x.value(), gradOutput);
            x.accumulateGradient(gradX);
        }
    }

    @Override
    public String name() {
        return "GELU";
    }
}